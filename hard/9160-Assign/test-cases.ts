...
type GetKeys<T extends unknown[]> = T extends [infer F, ...infer R]
  ? F extends Record<string, unknown>
    ? keyof F | GetKeys<R>
    : GetKeys<R>
  : never;

type GetValue<T extends unknown[], K extends PropertyKey> = T extends [...infer R, infer V]
  ? K extends keyof V
    ? V[K]
    : GetValue<R, K>
  : never;

type Assign<T, U extends unknown[]> = {
  [K in GetKeys<[T, ...U]>]: GetValue<[T, ...U], K>
}
